...
async function fetchExampleSentences(word) {
  ...
  const response = await fetch("/api/claude", {
    ...
    body: JSON.stringify({
      ...
      messages: [{
        role: "user",
        content: `You are a German language teacher helping a C1 level student. For the German word or expression: "${word}" Return ONLY a JSON object (no markdown, no backticks, no explanation) in this exact format: {"translation":"English translation","type":"noun / verb / expression / adjective / etc","explanation":"Brief explanation in 1-2 sentences","sentences":[{"german":"Example sentence","english":"Translation"},{"german":"Second example","english":"Translation"},{"german":"Third example","english":"Translation"}]}`
      }]
    })
  });
...
const typeColor = (type) => {
  ...
  if (t.includes("noun")) return { bg: "#3d2e1e", text: "#e8c49a" };
  if (t.includes("verb")) return { bg: "#1e2e3d", text: "#9ac4e8" };
  if (t.includes("expression") || t.includes("phrase")) return { bg: "#2e1e3d", text: "#c49ae8" };
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#0a0908", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", fontFamily:"'Palatino Linotype',Palatino,serif", padding:24 }}>
      <div style={{ textAlign:"center", maxWidth:380 }}>
        ...
        <p style={{ color:"#7a6e5e", fontSize:14, lineHeight:1.8, marginBottom:32 }}>
          Enter a personal PIN or passphrase to access your vocabulary.<br/>
          First time? Just pick any PIN — it creates your personal list.
        </p>
        <input
          value={pin}
          onChange={e => { setPin(e.target.value); setError(""); }}
          onKeyDown={e => e.key === "Enter" && handle()}
          placeholder="Your PIN or passphrase…"
          type="password"
...
export default function App() {
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      setWords(prev => [{
        id: inserted.id, word: inserted.word, translation: inserted.translation,
        type: inserted.type, explanation: inserted.explanation,
        sentences: inserted.sentences, mastered: inserted.mastered, addedAt: inserted.added_at
      }, ...prev]);
      setInput("");
  ...
  const toggleMastered = async (id, current) => {
    await sbFetch(`/rest/v1/vocabulary?id=eq.${id}`, {
      method: "PATCH",
      body: JSON.stringify({ mastered: !current })
    });
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#0a0908", fontFamily:"'Palatino Linotype',Palatino,serif", color:"#e8e0d0" }}>
      ...
      <div style={{ borderBottom:"1px solid #1a1815", padding:"20px 36px 16px", background:"#0a0908", position:"sticky", top:0, zIndex:10 }}>
        <div style={{ maxWidth:740, margin:"0 auto" }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14 }}>
            ...
            <button onClick={handleLogout} style={{ background:"transparent", border:"1px solid #252320", borderRadius:4, color:"#3a3830", fontSize:10, fontFamily:"inherit", letterSpacing:"0.08em", padding:"4px 12px", cursor:"pointer" }}>
              Lock
            </button>
          </div>
          <div style={{ display:"flex", gap:10 }}>
            <input
              ...
              placeholder="Enter a German word or expression…"
            ...
            <button onClick={handleAdd} disabled={loading || !input.trim()} style={{
              background: loading ? "#1a1915" : "#c8a96e",
              color: loading ? "#3a3830" : "#0a0908",
              border:"none", borderRadius:6, padding:"11px 22px", fontSize:12,
              fontFamily:"inherit", fontWeight:"bold", letterSpacing:"0.08em",
              textTransform:"uppercase", cursor: loading ? "not-allowed" : "pointer", whiteSpace:"nowrap"
            }}>
              {loading ? "Searching…" : "Add Word"}
            </button>
          </div>
      ...
      <div style={{ maxWidth:740, margin:"0 auto", padding:"16px 36px 0" }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
          <div style={{ display:"flex", gap:16, fontSize:11, color:"#3a3830", letterSpacing:"0.06em" }}>
            ...
            <span style={{ color:"#c8a96e" }}>{words.filter(w => w.mastered).length} mastered</span>
            <span>{words.filter(w => !w.mastered).length} learning</span>
          </div>
          <div style={{ display:"flex", gap:5 }}>
            {["all","learning","mastered"].map(f => (
              <button key={f} onClick={() => setFilter(f)} style={{
                background: filter===f ? "#c8a96e" : "transparent",
                color: filter===f ? "#0a0908" : "#3a3830",
                border:"1px solid", borderColor: filter===f ? "#c8a96e" : "#252320",
                borderRadius:4, padding:"3px 11px", fontSize:10,
                fontFamily:"inherit", letterSpacing:"0.08em", textTransform:"capitalize", cursor:"pointer"
              }}>{f}</button>
            ))}
          </div>
        </div>
      ...
      <div style={{ maxWidth:740, margin:"0 auto", padding:"12px 36px 60px" }}>
        {dbLoading && <div style={{ textAlign:"center", padding:"40px 0", color:"#3a3830", fontSize:13 }}>Loading your vocabulary…</div>}
        {!dbLoading && filteredWords.length === 0 && (
          <div style={{ textAlign:"center", padding:"60px 0", color:"#252320" }}>
            <div style={{ fontSize:34, marginBottom:10 }}>📖</div>
            <p style={{ fontSize:13, letterSpacing:"0.06em" }}>{words.length===0 ? "Add your first word to begin" : "No words in this category"}</p>
          </div>
        )}
        {filteredWords.map(w => {
            ...
            <div key={w.id} style={{ background:"#0e0d0b", border:"1px solid", borderColor:expandedId===w.id?"#2a2820":"#161512", borderRadius:7, marginBottom:7, overflow:"hidden", opacity:w.mastered?0.5:1, transition:"all 0.2s" }}>
              <div onClick={() => setExpandedId(expandedId===w.id ? null : w.id)} style={{ display:"flex", alignItems:"center", padding:"14px 16px", cursor:"pointer", gap:12 }}>
                ...
                <div style={{ display:"flex", gap:7, alignItems:"center", flexShrink:0 }}>
                  <button onClick={e => { e.stopPropagation(); toggleMastered(w.id, w.mastered); }} style={{ background:"transparent", border:"1px solid", borderColor:w.mastered?"#c8a96e":"#252320", color:w.mastered?"#c8a96e":"#2e2c26", borderRadius:4, padding:"3px 9px", fontSize:9, fontFamily:"inherit", letterSpacing:"0.08em", textTransform:"uppercase", cursor:"pointer" }}>
                    {w.mastered ? "✓ Mastered" : "Master"}
                  </button>
                  <button onClick={e => { e.stopPropagation(); handleDelete(w.id); }} onMouseEnter={e=>e.target.style.color="#c87070"} onMouseLeave={e=>e.target.style.color="#252320"}
              ...
              {expandedId === w.id && (
                <div style={{ borderTop:"1px solid #161512", padding:"16px 16px 20px" }}>
                  <p style={{ fontSize:13, color:"#7a6e5e", lineHeight:1.7, margin:"0 0 16px", fontStyle:"italic" }}>{w.explanation}</p>
                  <div style={{ fontSize:9, color:"#3a3830", letterSpacing:"0.12em", textTransform:"uppercase", marginBottom:10 }}>Example Sentences</div>
                  ...
                  <div style={{ fontSize:9, color:"#252320", marginTop:14, letterSpacing:"0.06em" }}>
                    Added {new Date(w.addedAt).toLocaleDateString("en-GB",{day:"numeric",month:"short",year:"numeric"})}
                  </div>
                </div>
              )}
            </div>
